...
import com.dimediary.persistence.converter.ContinuousTransactionTransformer;
...
import com.dimediary.persistence.repositories.cruds.RecurrenceExceptionCrudRepository;
import com.dimediary.persistence.repositories.cruds.RecurrenceExtraInstanceCrudReporitory;
...
class ContinuousTransactionRepoImpl implements ContinuousTransactionRepo {
  ...
  @Override
  public List<ContinuousTransaction> getContinuousTransactions(final UUID bankAccountId) {
    Validate.notNull(bankAccountId);

    ContinuousTransactionRepoImpl.log
        .info("geContinuousTransactions for bank account: " + bankAccountId);

    return this.continuousTransactionCrudRepository
        .getAllByBankAccountId(bankAccountId.toString())
        .stream().map(this.continuousTransactionTransformer::to)
  ...
  public ContinuousTransaction persist(final ContinuousTransaction continuousTransaction) {
    ...
    return this.continuousTransactionTransformer.to(continuousTransactionEntity);
  }
  ...
  public void delete(final ContinuousTransaction continuousTransaction) {
    ...
    com.dimediary.persistence.repositories.ContinuousTransactionRepoImpl.log
  ...
  public ContinuousTransaction getContinuousTransaction(final UUID continuousTransactionId) {
    return this.continuousTransactionTransformer.to(this.continuousTransactionCrudRepository
  ...
  private com.dimediary.persistence.entities.ContinuousTransactionEntity domainToEntity(
    ...
    final Collection<RecurrenceExceptionEntity> exceptionsToSave = continuousTransaction
        .getRecurrenceExceptions().stream().map(localDate -> {
    ...
    final Collection<RecurrenceExtraInstanceEntity> extraInstancesToSave = continuousTransaction
        .getRecurrenceExtraInstances().stream().map(localDate -> {
